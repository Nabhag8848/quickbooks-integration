...
export class BackfillProcessor extends WorkerHost {
  ...
  async process(job: Job<SyncJobDataDto>): Promise<void> {
    ...
    await this.syncStateService.markInitialBackfillInProgress(
      companySourceId,
      objectType
    );
  ...
  private async startInitialBackfill(
    ...
    await lastValueFrom(
      of({ startPosition: 1, hasMore: true }).pipe(
          ...
          return from(
            ...
            concatMap(async (response) => {
              ...
              return {
                ...
                hasMore:
                  response.hasMore && response.nextStartPosition !== undefined,
              };
            }),
            catchError((error) => {
              console.error(
                `Error during backfill for ${companySourceId}:${objectType}`,
        ...
        finalize(async () => {
          try {
            await this.syncStateService.markInitialBackfillCompleted(
              companySourceId,
              objectType
            );
            ...
            console.error(
              `Error in finalize for ${companySourceId}:${objectType}`,
              error
            );
          }
        })
